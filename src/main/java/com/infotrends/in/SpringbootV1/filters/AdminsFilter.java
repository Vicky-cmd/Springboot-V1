	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws IOException, ServletException {
		...
		if(excludedUrls.contains(path)) {
			...
			if(newSession!=null &&  newSession.getAttribute("username")!=null && !newSession.getAttribute("username").toString().isEmpty()) {
				if(newSession.getAttribute("isAdmin")!=null && newSession.getAttribute("isAdmin").toString().equalsIgnoreCase("Y")) {
					chain.doFilter(req, resp);
				} else {
					response.addHeader("forbidden-err-Msg", "Access Denied!");
					request.getRequestDispatcher(request.getContextPath() + "/forbidden").forward(request, response);;
